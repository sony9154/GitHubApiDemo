...
class PlayerVC: NodeVC,
    ...
    func setupPlayer() {
        ...
        guard let url = URL(string: self.node?.mediaItem.value?.previewUrl ?? "") else { return }
        ...
        playbackSlider.addTarget(self, action: #selector(PlayerVC.playbackSliderValueChanged(_:)), for: .valueChanged)
